<template>
	<div class="city city-wap">
		<div class="search">
			<input type="text" placeholder="支持首字母跟地名精确查找" value="" v-model="value" v-on:input="serach()">
		</div>
		<div class="city-list">
			<div class="block-60"></div>
			<h2 style="padding: 0.2rem 0;">当前城市:<span v-text="this.store.state.user.address"></span></h2>
			<div style="padding: 0.2rem 0;" v-show="showCity">没有搜索到结果</div>
			<div v-for="item in cityListData" class="clearfix">
				<p :id="item.ckey">{{item.ckey}}</p>
				<ul>
					<li v-for="ritem in item.cityList" @click="fnselect(ritem.airportName)">{{ritem.airportName}}</li>
				</ul>
			</div>
		</div>
		<div class="filter">
